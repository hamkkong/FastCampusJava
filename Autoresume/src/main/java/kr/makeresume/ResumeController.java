...
public class ResumeController {
    ...
    private void createResumeSheet(PersonInfo personInfo, List<Education> educationList, List<Career> careerList) {
        ...
        try (InputStream photoStream = new FileInputStream(photoFilename)) {
            ...
            int newWidth = (int) (35 * 2.83465); // mm단위를 픽셀 단위로 변환 (1mm=2.83465px)
            int newHeight = (int) (45 * 2.83465);
            Image resizedImage = originalImage.getScaledInstance(newWidth, newHeight, Image.SCALE_SMOOTH);
            BufferedImage resizedBufferdImage = new BufferedImage(newWidth, newHeight, BufferedImage.TYPE_4BYTE_ABGR);
            Graphics2D g2d = resizedBufferdImage.createGraphics();
            g2d.drawImage(resizedImage, 0, 0, null);
            g2d.dispose();

            //조절된 이미지를 바이트 배열로 변환
            ByteArrayOutputStream baos = new ByteArrayOutputStream();
            ImageIO.write(resizedBufferdImage, "png", baos);

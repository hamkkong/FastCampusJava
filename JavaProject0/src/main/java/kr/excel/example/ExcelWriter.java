...
public class ExcelWriter {
    public static void main(String[] args) {
        // API를 사용하여 고객의 니즈에 맞는 형태의 출력물을 빠르게 받아볼 수 있도록 하는 노력이 필요하다.
        Scanner scanner = new Scanner(System.in);
        List<MemberVO> members = new ArrayList<>();
        // 키보드로부터 데이터를 받아오는 코드 작성
        ...
        while (true){
            ...
            scanner.nextLine(); //개행문자 제거 (이걸 넣지 않으면 엔터치면 다음으로 바로 넘어감)
            ...
            MemberVO member = new MemberVO(name, age, birthday, phone, address, isMarried);
            members.add(member);

        }
        scanner.close();  // 여기까지 정보입력에 관한 코드
        try{

...
public class PdfExporter {
    public static void exportToPdf(String date, List<CovidStatus>covidStatusList, String fileName) throws FileNotFoundException{
        ...
        try{
            //한글 폰트 파일 경로를 지정
            String KoreanFontPath = "C:/Users/ihyemi/Library/Fonts/NanumGothicLight.ttf";
            ...
            for(CovidStatus covidStatus : covidStatusList){
                ...
                table.addCell(new Paragraph(String.format("%.2f", covidStatus.getRate())).setFont(koreanFont));
            }

            //테이블을 문서에 추가하고 문서를 닫기
            doc.add(table);
            doc.close();
